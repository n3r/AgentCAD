...
The first four verbs are registry passthroughs, because they are exactly the
agent surface (``list_comments`` / ``add_comment`` / ``resolve_thread`` /
``reopen_thread``) and one implementation must serve both. The last four are
**not tools on purpose**: reading one thread, editing or deleting your own
comment and reading an audit log are panel affordances, and FR7 freezes the
agent surface at five tools. They call ``service.comments`` directly, whose
structured errors the app's ``AppError`` handler maps identically.
