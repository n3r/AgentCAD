...
| 2 | **005-lite → [PRD-005a](prd/in-progress/PRD-005a-hosted-core.md)** ← in progress | Deploy + identity + public read only. Orgs, roles, audit principals and local-first sync stay deferred as genuine deployment work. Carved out on 17 Aug 2026 as its own PRD (031a/031b's letter-suffix precedent) so folder-as-status stays truthful for both halves. Its design settles the question the sequencing forced: **without PRD-006 an account is arbitrary code execution on the host**, so registration is closed, roles are not a boundary between members, and the anonymous surface is nine pre-generated, provably kernel-free entries. |
| 3 | **007** | Share links & customizer — the growth loop, and 031's own hard dependency. Needs hosting, not full multi-tenancy. Inherits from 005a: the `PUBLIC_PATHS` enumeration and its drift test (007's own AC9, delivered early), the route-pack `PREFIX` seam that lets `/s/<token>` mount at the root, the `TokenBucket` rate-limit primitive, and the recorded verdict that **bounded params on a member-authored script is a different threat from arbitrary upload** and is shippable before 006. |
...
| [005a](prd/in-progress/PRD-005a-hosted-core.md) | Hosted core ("005-lite") — one deployable instance (Dockerfile + compose), invite-only identity (sessions for browsers, bearer tokens for agents), and an enumerated, kernel-free public-read surface | **in-progress — step 2 of the marketplace chain** ([design](superpowers/specs/2026-08-17-hosted-core-design.md) · [plan](superpowers/plans/2026-08-17-hosted-core.md)) | founder decision (16 Aug 2026) | 011 · 008 |
