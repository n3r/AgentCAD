...
| `use_part` | **project, package, part, part_id**, preset, params | Materialise a package part into the project as an ordinary part, under an immutable provenance header. **Never touches an index or the network**; re-verifies the whole cached tree every time; re-materialising is byte-identical. Returns the ordinary `get_part` payload plus `package_provenance`. |
| `validate_package` | **path**, strict, stages, jobs, work_dir, budget_s | Run the nine-stage gate over a package directory and return the PRD-004-shaped report — no publish, no install, no side effect outside its own throwaway cell. **This is the authoring loop**: read `stages[].items[]`, fix, validate again. |
