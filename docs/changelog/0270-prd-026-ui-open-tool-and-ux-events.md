# 0270 — PRD-026 slice 5: the `ui_open` tool pack and `POST /api/ui/events`
...
**4676 passed, 44 skipped** (11 reported: the 9 `*_count_is_cited` guards read this very entry before its count was filled, `test_checks_pipeline` asserts a clean tree while the slice was uncommitted, and `test_checks_cli`'s 1 ms `--budget` race lost to a machine running two other suites — see 0271).
