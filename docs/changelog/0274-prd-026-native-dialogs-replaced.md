# 0274 — PRD-026 slice 2: every native prompt/confirm is gone; the legacy modals adopt the shell
...
- **The shortcut machine path is now total** — a bug the browser pass found,
  ...
  family as the `+` keystroke fixed in 0271's review round 1, closed at the
  root this time.
...
- `Mod+N` is `when`-gated, not `enabled`-gated: with no project open there is
  ...
  the opposite of the `Mod+S` ruling (0271 I2) and deliberately so — v0.1 never
